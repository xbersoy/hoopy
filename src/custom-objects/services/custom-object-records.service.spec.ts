...
describe('CustomObjectRecordsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: 'CustomObjectRecordRepository', useValue: mockRecordRepository },
        { provide: CustomObjectDefinitionsService, useValue: mockDefinitionsService },
    ...
    service = module.get<CustomObjectRecordsService>(CustomObjectRecordsService);
    jest.clearAllMocks();
  });
  ...
  describe('RBP Interactions', () => {
    it('should strip away fields missing read permissions', async () => {
      ...
      mockPermissionsService.hasPermissionByResourceType.mockImplementation(
        async (_ids: string[], action: string, resourceType: string) => {
          if (resourceType === 'co:TEST_OBJ:visibleTopSecret' && action === 'read') return false;
          return true;
        },
      );

      const record = { data } as any;
      const result = await (service as any).filterReadFields(record, definition, userPermissionIds);
    ...
    it('should allow fields when user has read permission', async () => {
      ...
      mockPermissionsService.hasPermissionByResourceType.mockResolvedValue(true);
    ...
    it('should throw BadRequestException if writing to edit-restricted field without permission', async () => {
      ...
      mockPermissionsService.hasPermissionByResourceType.mockResolvedValue(false);

      await expect(
        (service as any).enforceEditPermissions(payloadData, definition, userPermissionIds),
  ...
  describe('Data Validations', () => {
    it('should validate ATTACHMENT type by hitting AttachmentsService', async () => {
      const companyId = 'co1';
      const fields: any[] = [
        { code: 'file', dataType: CustomFieldType.ATTACHMENT, isRequired: true }
      ];
      const data = { file: 'attachment-uuid-xyz' };

      // Mock the attachment being found
      mockAttachmentsService.findOne.mockResolvedValueOnce({ id: 'attachment-uuid-xyz' });

      await expect((service as any).validateData(data, fields, companyId)).resolves.not.toThrow();
      expect(mockAttachmentsService.findOne).toHaveBeenCalledWith('attachment-uuid-xyz');
    });
    ...
    it('should throw if ATTACHMENT is missing (isRequired)', async () => {
      ...
      await expect((service as any).validateData(data, fields, companyId)).rejects.toThrow(BadRequestException);
    });

    it('should validate PICKLIST type by hitting PicklistsService', async () => {
      const companyId = 'co1';
      const fields: any[] = [
        { code: 'status', dataType: CustomFieldType.PICKLIST, picklistId: 'p-uuid', isRequired: true }
      ];
      const data = { status: 'INVALID_OPTION' };

      mockPicklistsService.isValidOptionCode.mockResolvedValueOnce(false);

      await expect((service as any).validateData(data, fields, companyId)).rejects.toThrow('Field "status" contains an invalid picklist option code');
      expect(mockPicklistsService.isValidOptionCode).toHaveBeenCalledWith('p-uuid', companyId, 'INVALID_OPTION');
    });

    it('should validate CUSTOM_OBJECT linking', async () => {
      const companyId = 'co1';
      const fields: any[] = [
        { code: 'assetRef', dataType: CustomFieldType.CUSTOM_OBJECT, referencedDefinitionId: 'def2' }
      ];
    ...
    it('should validate STRING_ARRAY type accepts valid string arrays', async () => {
      const companyId = 'co1';
      const fields: any[] = [
        { code: 'tags', dataType: CustomFieldType.STRING_ARRAY, isRequired: true }
      ];
    ...
    it('should reject STRING_ARRAY when value is not an array', async () => {
      ...
      await expect((service as any).validateData(data, fields, companyId)).rejects.toThrow(
    ...
    it('should reject STRING_ARRAY with non-string elements', async () => {
      const companyId = 'co1';
      const fields: any[] = [
        { code: 'tags', dataType: CustomFieldType.STRING_ARRAY, isRequired: false }
      ];
    ...
    it('should validate NUMBER_ARRAY type accepts valid number arrays', async () => {
      const companyId = 'co1';
      const fields: any[] = [
        { code: 'scores', dataType: CustomFieldType.NUMBER_ARRAY, isRequired: true }
      ];
    ...
    it('should reject NUMBER_ARRAY when value is not an array', async () => {
      ...
      await expect((service as any).validateData(data, fields, companyId)).rejects.toThrow(
        'Field "scores" must be an array of numbers',
      );
    });
    ...
    it('should reject NUMBER_ARRAY with non-number elements', async () => {
      const companyId = 'co1';
      const fields: any[] = [
        { code: 'scores', dataType: CustomFieldType.NUMBER_ARRAY, isRequired: false }
      ];
