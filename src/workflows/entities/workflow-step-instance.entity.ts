...
import { WorkflowStepInstanceStatus, WorkflowStepType, ApprovalStrategy } from '../enums/workflow.enums';
...
export class WorkflowStepInstance {
  ...
  @ManyToOne(() => WorkflowInstance, (i) => i.stepInstances, { onDelete: 'CASCADE' })
  ...
  @ApiProperty({ description: 'Reference to the step definition this was created from' })
  @Column({ type: 'uuid' })
  stepDefinitionId: string;

  @ApiProperty({ description: 'Step name (copied from definition for historical stability)' })
  @Column()
  name: string;
  ...
  @ApiProperty({ description: 'Current step status', enum: WorkflowStepInstanceStatus })
  ...
  @ApiProperty({ description: 'When this step was completed/resolved', required: false })
  @Column({ type: 'timestamptz', nullable: true })
  completedAt: Date;
  ...
  @ApiProperty({ description: 'When a reminder was last sent', required: false })
  @Column({ type: 'timestamptz', nullable: true })
  reminderSentAt: Date;
  ...
  @ApiProperty({ description: 'Decision/result of the step action', required: false })
  @Column({ nullable: true })
  decision: string;

  @ApiProperty({ description: 'Comment left during the action', required: false })
  @Column({ type: 'text', nullable: true })
  comment: string;

  @ApiProperty({ description: 'Additional metadata/form data collected at this step', required: false })
  @Column({ type: 'jsonb', nullable: true })
  metadata: Record<string, any>;

  @ApiProperty({
    description: 'Snapshot of the step definition config (for historical stability)',
    required: false,
  })
  @Column({ type: 'jsonb', nullable: true })
  definitionSnapshot: Record<string, any>;

  @OneToMany(() => WorkflowStepAssignee, (a) => a.stepInstance, { cascade: true })
